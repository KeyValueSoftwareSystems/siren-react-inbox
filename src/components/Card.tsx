...
const Card: FC<NotificationCardProps> = ({
  ...
  const activeCardContainer = {
    borderLeft: `4px ${styles.activeCardMarker.backgroundColor} solid`,
  };
  ...
  return (
    <div
      style={{
        ...
        ...(isRead ? {} : activeCardContainer),
      }}
      className={`${isRead ? "siren-sdk-card-transparent" : ""}`}
      onClick={() =>
        onNotificationCardClick && onNotificationCardClick(notification)
      }
      data-testid={`card-${notification.id}`}
    >
      <div className="siren-sdk-card-flex-container">
        {!cardProps?.hideAvatar && (
          <div className="siren-sdk-card-avatar-image-container">
            <img src={avatar?.imageUrl} alt="" style={styles.cardIconRound} />
          </div>
        )}
        <div
          className="siren-sdk-card-content-wrapper"
          style={{
            width: cardProps?.hideAvatar ? "100%" : "80%",
          }}
        >
          <div>
            <p style={styles.cardTitle} className="siren-sdk-text-break">
              {header}
            </p>
            <p style={styles.cardDescription} className="siren-sdk-text-break">
              {subHeader}
            </p>
            <p
              style={styles.cardDescription}
              className="siren-sdk-card-description siren-sdk-text-break"
            >
              {body}
            </p>
          </div>
          <div className="siren-sdk-card-flex-container">
            <TimerIcon color={styles.timerIcon.color} />
            <p style={styles.dateStyle}>{generateElapsedTimeText(createdAt)}</p>
          </div>
        </div>
      </div>
      <div
